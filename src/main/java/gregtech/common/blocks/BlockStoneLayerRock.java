...
/**
 * GT6-style surface indicator rocks placed during stone layer generation.
 * Similar to BlockSurfaceRock but drops rockGt items instead of dustTiny,
 * and can represent any material with a dust property (including stone types).
 */
@SuppressWarnings("deprecation")
public abstract class BlockStoneLayerRock extends BlockMaterialBase {
    ...
    public static final ModelResourceLocation MODEL_LOCATION = new ModelResourceLocation(
            GTUtility.gregtechId("stone_layer_rock"), "normal");

    public static BlockStoneLayerRock create(Material[] materials) {
        PropertyMaterial property = PropertyMaterial.create("variant", materials);
        return new BlockStoneLayerRock() {
    ...
    private BlockStoneLayerRock() {
        super(net.minecraft.block.material.Material.GOURD);
        setTranslationKey("stone_layer_rock");
        setHardness(0.25f);
    }
    ...
    private ItemStack getDropStack(IBlockState state, int amount) {
        ...
        // Try to drop rockGt item first, fall back to dustTiny
        ItemStack rockStack = OreDictUnifier.get(OrePrefix.rockGt, material, amount);
